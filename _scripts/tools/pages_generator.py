...
TAG_AND_CATEGORY_RE = re.compile(r'[a-z0-9\-]+')
...
def generate_category_pages(is_verbose):
    ...
    for category in categories:
        if TAG_AND_CATEGORY_RE.match(category) is None:
            raise ValueError(TAG_AND_CATEGORY_RE)
...
def generate_tag_pages(is_verbose):
    ...
    for tag in all_tags:
        if TAG_AND_CATEGORY_RE.match(tag) is None:
            raise ValueError(TAG_AND_CATEGORY_RE)
